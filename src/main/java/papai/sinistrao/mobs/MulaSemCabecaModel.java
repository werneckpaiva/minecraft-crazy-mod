...
public class MulaSemCabecaModel extends ModelBase{
...
//        this.setBoxRotation(this.muleRightEar, 0.5235988F, 0.0F, -0.2617994F);
     ...
     */
    public void render(Entity entity, float param1, float param2, float param3, float p_78088_5_, float param4, float renderParam)
    {
        MulaSemCabecaEntity mulaSemCabecaEntity = (MulaSemCabecaEntity)entity;
        int horseType = mulaSemCabecaEntity.getHorseType();
        float grassEatingAmount = mulaSemCabecaEntity.getGrassEatingAmount(0.0F);
        boolean isAdult = mulaSemCabecaEntity.isAdultHorse();
        boolean isChested = isAdult && mulaSemCabecaEntity.isChested();
        boolean isMule = horseType == 1 || horseType == 2;
        float horseSize = mulaSemCabecaEntity.getHorseSize();
        boolean isNotHidden = mulaSemCabecaEntity.riddenByEntity != null;

        this.backLeftLeg.render(renderParam);
        this.backLeftShin.render(renderParam);
        this.backLeftHoof.render(renderParam);
        this.backRightLeg.render(renderParam);
        this.backRightShin.render(renderParam);
        this.backRightHoof.render(renderParam);
        this.frontLeftLeg.render(renderParam);
        this.frontLeftShin.render(renderParam);
        this.frontLeftHoof.render(renderParam);
        this.frontRightLeg.render(renderParam);
        this.frontRightShin.render(renderParam);
        this.frontRightHoof.render(renderParam);

        this.body.render(renderParam);
        this.tailBase.render(renderParam);
        this.tailMiddle.render(renderParam);
        this.tailTip.render(renderParam);
        this.neck.render(renderParam);
        this.mane.render(renderParam);

//        this.muleLeftEar.render(renderParam);
//        this.muleRightEar.render(renderParam);

//        this.head.render(renderParam);

        if (isChested)
        {
            this.muleLeftChest.render(renderParam);
            this.muleRightChest.render(renderParam);
        }
    }
     ...
     */
    private void setBoxRotation(ModelRenderer modelRenderer, float angleX, float angleY, float angleZ)
    {
    ...
    public void setLivingAnimations(EntityLivingBase entity, float p_78086_2_, float p_78086_3_, float p_78086_4_)
    {
        ...
        float f6 = f4 - f3;
        float angleX = f5 / (180F / (float)Math.PI);
        ...
        if (p_78086_3_ > 0.2F)
        {
            angleX += MathHelper.cos(p_78086_2_ * 0.4F) * 0.15F * p_78086_3_;
        }

        MulaSemCabecaEntity entityhorse = (MulaSemCabecaEntity) entity;
...
//        this.head.rotationPointZ = -10.0F;
        ...
        float rotationX = 0.5235988F + angleX;
        float rotationY = 6 / (180F / (float)Math.PI);
        rotationX = f9 * (0.2617994F + angleX) + f8 * 2.18166F + (1.0F - Math.max(f9, f8)) * rotationX;
        rotationY = f9 * (f6 / (180F / (float)Math.PI)) + (1.0F - Math.max(f9, f8)) * rotationY;
        float rotationPointY = 4.0F;
        float rotationPointZ = -10.0F;
        rotationPointY = f9 * -6.0F + f8 * 11.0F + (1.0F - Math.max(f9, f8)) * rotationPointY;
        rotationPointZ = f9 * -1.0F + f8 * -10.0F + (1.0F - Math.max(f9, f8)) * rotationPointZ;
        
//        this.head.rotateAngleX = 0.5235988F + angleX;
//        this.head.rotateAngleY = f6 / (180F / (float)Math.PI);
//        this.head.rotateAngleX = f9 * (0.2617994F + angleX) + f8 * 2.18166F + (1.0F - Math.max(f9, f8)) * this.head.rotateAngleX;
...
//        this.muleRightEar.rotationPointY = this.head.rotationPointY;
        this.neck.rotationPointY = rotationPointY;
...
//        this.muleRightEar.rotationPointZ = this.head.rotationPointZ;
        this.neck.rotationPointZ = rotationPointZ;
...
//        this.muleRightEar.rotateAngleX = this.head.rotateAngleX;
        this.neck.rotateAngleX = rotationX;
...
//        this.muleRightEar.rotateAngleY = this.head.rotateAngleY;
        this.neck.rotateAngleY = rotationY;
...
//            this.horseRightFaceMetal.rotationPointZ = this.head.rotationPointZ;
            this.horseLeftRein.rotateAngleX = angleX;
            this.horseRightRein.rotateAngleX = angleX;
