...
public class Dimençaaether {
	...
	.setBlockName("d_portal")
	.setBlockTextureName("wool_colored_white"));
	Block.blockRegistry.addObject(199, "d_portal", portal);
	block = (ModTrigger)(new ModTrigger().setUnlocalizedName("d_trigger").setTextureName("Mega_stell"));
	Item.itemRegistry.addObject(451, "d_trigger", block);
	}

	public Dimençaaether(){}

	public void load(){
	GameRegistry.registerBlock(portal, "d_portal");
	DimensionManager.registerProviderType(DIMID, Dimençaaether.WorldProviderMod.class, false);
	DimensionManager.registerDimension(DIMID, DIMID);

	GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]{
		"XXX", "XX5", "X7X", Character.valueOf('5'), new ItemStack(Items.water_bucket, 1), Character.valueOf('7'), new ItemStack(Items.flint, 1), 
	});
	...
	public static class WorldProviderMod extends WorldProvider{
	    ...
	    public IChunkProvider createChunkGenerator(){
	        return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed()-5922);
	    }
	        ...
	        for (i3 = 0; i3 < 4; ++i3)
	        {
	            for (j3 = 0; j3 < 4; ++j3)
	            {
	                for (k3 = -1; k3 < 4; ++k3)
	                {
	                    ...
	                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? Blocks.glowstone : portal), 0, 2);
	                }
	            }
	...
	this.gor = reg.registerIcon("wool_colored_white");
	this.dol = reg.registerIcon("wool_colored_white");
	this.st1 = reg.registerIcon("wool_colored_white");
	this.st2 = reg.registerIcon("wool_colored_white");
	this.st3 = reg.registerIcon("wool_colored_white");
	this.st4 = reg.registerIcon("wool_colored_white");
	}
	...
	*/
	...
	byte b0 = 0;
	byte b1 = 0;
	if (par1World.getBlock(par2 - 1, par3, par4) == Blocks.glowstone || par1World.getBlock(par2 + 1, par3, par4) == Blocks.glowstone)
	{
	b0 = 1;
	}
	if (par1World.getBlock(par2, par3, par4 - 1) == Blocks.glowstone || par1World.getBlock(par2, par3, par4 + 1) == Blocks.glowstone)
	{
	b1 = 1;
	}
	if (b0 == b1)
	...
	*/
	...
	if (par1World.getBlock(par2, i1 - 1, par4) != Blocks.glowstone)
	{
	...
	if (j1 == 3 && par1World.getBlock(par2, i1 + j1, par4) == Blocks.glowstone)
	{
	...
	{
	         if ((par1World.getBlock(par2 + b0, par3, par4 + b1) != Blocks.glowstone || par1World.getBlock(par2 - b0, par3, par4 - b1) != this) && (par1World.getBlock(par2 - b0, par3, par4 - b1) != Blocks.glowstone || par1World.getBlock(par2 + b0, par3, par4 + b1) != this))
	         {
	         par1World.setBlockToAir(par2, par3, par4);
	         }
	}
	...
	public static class ChunkProviderModded implements IChunkProvider
	{
	                    ...
	                    for (int l2 = 0; l2 < 8; ++l2)
	                    {
	                        ...
	                        for (int i3 = 0; i3 < 4; ++i3)
	                        {
	                            ...
	                            for (int k3 = 0; k3 < 4; ++k3)
	                            {
	                                if ((d15 += d16) > 0.0D)
	                                {
	                                    p_147424_3_[j3 += short1] = Blocks.glowstone;
	                                }
	                                else if (k2 * 8 + l2 < b0)
	                                {
	                                    p_147424_3_[j3 += short1] = Blocks.quartz_block;
	                                }
	                                else
	                                {
	    ...
	    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	    {
	            ...
	            && TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAKE))
	        {
	            ...
	            (new WorldGenLakes(Blocks.quartz_block)).generate(this.worldObj, this.rand, k1, l1, i2);
	        }
